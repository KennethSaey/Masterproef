...
import masterproef.ScalaTimer
import masterproef.TimerEvent
...
class Gameboard(window: MainFrame, var width: Int, var height: Int) extends Component with Reactor {
	...
	val timer = new ScalaTimer(2000)

	var zoom: Boolean = false

	val player1: String = Game.currentPlayer.name
	val player2: String = Game.currentOpponent.name
	val maxDeckCards: Int = 10
	var deck1: Int = Game.currentPlayer.deck.size
	var hand1: Int = Game.currentPlayer.hand.size
	var battlefield1: Int = Game.currentPlayer.battlefield.size
	var graveyard1: Int = Game.currentPlayer.graveyard.size
	var hover1: Int = -1
	var deck2: Int = Game.currentOpponent.deck.size
	var hand2: Int = Game.currentOpponent.hand.size
	var battlefield2: Int = Game.currentOpponent.battlefield.size
	var graveyard2: Int = Game.currentOpponent.graveyard.size
	...
	override def paintComponent(g: Graphics2D) {
		...
		if (text != "") {
			...
			g.setFont(new Font("Cambria", Font.PLAIN, 64))
			val textWidth = g.getFontMetrics().stringWidth(text) / 2
			val textHeight = g.getFontMetrics().getHeight() / 2
			g.drawString(text, width / 2 - textWidth, height / 2 + textHeight)
		}
	}
