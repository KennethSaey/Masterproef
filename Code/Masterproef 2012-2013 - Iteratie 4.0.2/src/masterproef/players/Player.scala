...
//		println("Adding to " + name + "s deck...")
	...
	def move(card: Card, from: Deck, to: Deck/*, newState: CardState.Value*/) = {
//		println("Moving " + card)
...
//		println(hand.size + "-Playing " + card)
		...
		Masterproef.model.publish(new PlayEvent(card))
...
//		}
	...
	def destroy(card: Card): Unit = {
		move(card, hand, graveyard)
	}
